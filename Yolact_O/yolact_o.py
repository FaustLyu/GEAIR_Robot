...
class PredictionModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=1024, aspect_ratios=[[1]], scales=[1], parent=None, index=0):
        ...
        self.mask_dim = cfg.mask_dim  # Defined by Yolact_O
        self.num_priors = sum(len(x) * len(scales) for x in aspect_ratios)
        self.parent = [parent]  # Don't include this in the state dict
        self.index = index
        self.num_heads = cfg.num_heads  # Defined by Yolact_O
        self.num_ori = 5
        ...
        if parent is None:
            ...
            self.theta_layer = nn.Conv2d(out_channels, self.num_priors, **cfg.head_layer_params)
            self.ori_layer = nn.Conv2d(out_channels, self.num_priors * self.num_ori, **cfg.head_layer_params)
            ...
            ###
            self.bbox_extra, self.conf_extra, self.mask_extra, self.theta_extra, self.ori_extra = [make_extra(x) for x in cfg.extra_layers]
    ...
    def forward(self, x):
        ...
        ###
        theta_x = src.theta_extra(x)
        ori_x = src.ori_extra(x)
        ...
        ### layers
        theta = src.theta_layer(theta_x).permute(0, 2, 3, 1).contiguous().view(x.size(0), -1, 1)
        ori = src.conf_layer(ori_x).permute(0, 2, 3, 1).contiguous().view(x.size(0), -1, self.num_ori)
        ###
...
class Yolact_O(nn.Module):
    ...
    def __init__(self):
        ...
        # Compute mask_dim here and add it back to the config. Make sure Yolact_O's constructor is called early!
        if cfg.mask_type == mask_type.direct:
    ...
    def forward(self, x):
        ...
        with timer.env('pred_heads'):
            ###
            pred_outs = {'loc': [], 'conf': [], 'mask': [], 'priors': [], 'theta': [], 'ori': []}
        ...
        if self.training:
            ...
            if cfg.use_focal_loss:
                if cfg.use_sigmoid_focal_loss:
                    ...
                    ###
                    pred_outs['ori'] = torch.sigmoid(pred_outs['ori'])
                    if cfg.use_mask_scoring:
                        pred_outs['conf'] *= pred_outs['score']
                        pred_outs['ori'] *= pred_outs['ori']
                    ...
                    ###
                    pred_outs['ori'] = F.softmax(pred_outs['ori'], -1)
            else:
